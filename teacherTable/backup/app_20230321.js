...
function clicked(event) {
  if (event.target.getAttribute("rowspan") !== null) {
    ...
    if (event.target.tagName === "DIV") {
      ...
      textareaElement.classList.add("selected");
      textareaElement.value = event.target.innerHTML;
      const tdElement = event.target.parentNode;
      event.target.parentNode.innerHTML = "";
      tdElement.appendChild(textareaElement);
      textareaElement.focus();
    } else {
      const divElement = document.createElement("div");
      divElement.innerHTML = event.target.value;
      const tdElement = event.target.parentNode;
      event.target.parentNode.innerHTML = "";
      if (event.target.value.length > 0) {
        divElement.classList.add("contentTd");
      }
...
//   let nameOfLsDataOfThisTable = currentPage + "tableData" + this.parentNode.id.slice(-1);
//   console.log(nameOfLsDataOfThisTable);
//   this.parentNode.remove();
//   localStorage.removeItem(nameOfLsDataOfThisTable);
...
function printTable() {
  ...
  for (let k = 0; k < JSON.parse(localStorage.getItem("sheetList")).length; k++) {
    ...
    for (let p = 0; p < numOfTables; p++) {
      ...
      if (localStorage.getItem("teachers" + currentPage) === null || JSON.parse(localStorage.getItem("teachers" + currentPage)).length < numOfTables) {
      } else {
        if (JSON.parse(localStorage.getItem("teachers" + currentPage))[p] === "undefined" || JSON.parse(localStorage.getItem("teachers" + currentPage))[p] === "") {
        } else {
      ...
      for (let i = 0; i < 8; i++) {
        if (i === 5) {
          ...
          for (let j = 0; j < 21; j++) {
            ...
            let txtElement2 = document.createElement("div");
            if (j === 20) {
